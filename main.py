...
def valid_space(shape, grid):
    ...
    accepted_pos = [j for sub in accepted_pos for j in sub]  # przejscie z 2d do 1d
...
def draw_grid(surface, grid):
    ...
    for i in range(len(grid)):
        pygame.draw.line(surface, (0, 200, 100), (sx, sy + i * block_size), (sx + play_width, sy + i * block_size))
        for j in range(len(grid[i])):
            pygame.draw.line(surface, (0, 200, 100), (sx + j * block_size, sy), (sx + j * block_size, sy + play_height))
...
def main(win):
    ...
    while run:
        ...
        if check_lost(locked_positions):
            draw_text_middle(win, " You lost :( ", 80, (200, 100, 50))
            pygame.display.update()
...
def main_menu(win):
    ...
    while run:
        win.fill((0, 0, 0))
        draw_text_middle(win, 'Press any key to play', 60, (50, 100, 150))
        pygame.display.update()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                run = False
